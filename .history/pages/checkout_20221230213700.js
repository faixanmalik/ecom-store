...
function Checkout({cart , subTotal, removeFromCart, addToCart }) {
  ...
  <section className="text-gray-600 body-font relative">
    <div className="container px-5 py-24 mx-auto w-9/12">
          ...
          <form method='POST' onSubmit={submit}>
            ...
            <div className="relative">
              ...
              <select onChange={handleChange} value={country} id="country" name="country" autoComplete="country-name" className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                <option onChange={handleChange}>United States</option>
                <option onChange={handleChange}>Pakistan</option>
                <option onChange={handleChange}>Canada</option>
                <option onChange={handleChange}>Mexico</option>
              </select>
            </div>
            </div>
      ...
      <div className="pt-3 cart rounded-lg bg-blue-100 px-7 py-7">
          ...
          {Object.keys(cart).map((item)=>{ return <li key={item} className="text-lg">
            <div className="flex justify-between">
              <div className="w-1/2 overflow-hidden">{cart[item].name}({cart[item].size}/{cart[item].variant})</div>
                <div className="bg-slate-100 flex text-blue-800 h-8 font-bold text-center px-2"><AiFillMinusCircle onClick={()=>{removeFromCart(item,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> <span className='mx-3'>{cart[item].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(item,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> </div>
            </div>  
          </li>})} 
        </ol>
        <div>
            <button className='text-lg font-semibold py-2 rounded'>SubTotal: ${subTotal}</button>
        </div>
      </div>
      {/* <button type='submit' className='bg-blue-600 mt-4 px-5 py-2 flex rounded text-white'><AiOutlineShopping className='my-auto mx-2'/><Link href={"/order"}>Pay ${subTotal}</Link></button> */}
      <button type='submit' className='bg-blue-600 mt-4 px-5 py-2 flex rounded text-white'><AiOutlineShopping className='my-auto mx-2'/>Pay ${subTotal}</button>
