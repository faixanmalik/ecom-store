  ...
  <section className="text-gray-600 body-font">
    <div className="container px-5 pt-5 pb-20 mx-auto">
      <div className="flex flex-wrap px-10 lg:px-20 space-y-20">
        ...
        {Object.keys(product).length === 0 && <div className="">Sorry! Currently Stock Unavailble right now. Please wait for new Stocks</div>}
        {Object.keys(product).map((item)=>{
        return <div key={item._id} className="w-64 mx-auto">
          <div className="block h-72 rounded overflow-hidden">
            <Link href={`/product/${item.slug}`}><img alt="ecommerce" className="w-full h-full block" src={products[item].img}/></Link>
          </div>
