...
function Order({subTotal, cart }) {
  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      ...
      {/* {Object.keys(cart).map((item)=>{
        return <img key={item} alt="orders pic" className="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded" src={cart[item].img}/>
      })}  */}
      <img alt="orders pic" className="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded" src={cart[item].img}/>
    </div>
  </div>
</section>
  )
