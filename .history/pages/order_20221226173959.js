    ...
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      ...
      {Object.keys(cart).map((item)=>{ 
        ...
        return <div key={item} className="flex w-full border-b-2 border-gray-200 py-2">
          <div className="w-1/3 text-center text-gray-500">{cart[item].name}( {cart[item].size}/{cart[item].variant} )</div>  
          <div className="w-1/3 text-center text-gray-900">{cart[item].quantity}</div>
          <div className="w-1/3 text-center text-gray-900">{cart[item].price}</div>
        </div>})} 
