    ...
    <header className="text-gray-600 body-font border-b-2 ">
        <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
            ...
            <div className="z-10">
              ...
              <div ref={ref} className="pt-3 cart rounded-lg absolute top-0 right-0 bg-blue-100 px-7 w-96 h-full transform transition-transform translate-x-full">
                  ...
                  <ol className="pt-7 list-decimal font-medium text-red-800 space-y-2">
                    ...
                    {Object.keys(cart).map((k)=>{ return <li key={k} className="text-lg">
                      <div className="flex justify-between">
                        ...
                        <div className="bg-slate-100 flex text-blue-800 h-auto font-bold text-center px-2"><AiFillMinusCircle onClick={()=>{removeFromCart(k,cart[k].name,1,cart[k].price,cart[k].size,cart[k].variant)}} className='my-auto cursor-pointer'/> <span className='mx-3'>{cart[k].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(k,cart[k].name,1,cart[k].price,cart[k].size,cart[k].variant)}} className='my-auto cursor-pointer'/> </div>
                      </div>
                      <div>
                      </div>
                    </li>})}
