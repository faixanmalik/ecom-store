                  ...
                  <ol className="pt-7 list-decimal font-medium text-red-800 space-y-2">
                    ...
                    {Object.keys(cart).map((item)=>{  return localStorage.getItem('token') ? <li  key={item} className="text-lg text-blue-800">
                      <div className="flex justify-between">
                        ...
                        <div className="bg-slate-100 flex h-8 font-bold text-center px-2"><AiFillMinusCircle onClick={()=>{removeFromCart(k,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> <span className='mx-3'>{cart[item].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(k,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> </div>
                      </div>
                      <div>
                      </div>
