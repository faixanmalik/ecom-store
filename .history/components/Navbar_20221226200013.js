...
function Navbar({logout, visibleCart , user, cart, removeFromCart, addToCart, clearCart, subTotal , size , variant}) {
  ...
  return (
    <header className="text-gray-600 body-font border-b-2 ">
        <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
            ...
            <div className="z-10">
              <div ref={ref} className="pt-3 overflow-scroll cart rounded-lg absolute top-0 right-0 bg-blue-100 px-7 w-2/5 h-full transform transition-transform translate-x-full">
                ...
                { (visibleCart) ?  <div>
                    <ol className="pt-7 list-decimal font-medium text-red-800 space-y-2">
                      ...
                      {Object.keys(cart).map((item)=>{  return <li  key={item} className="text-lg text-blue-800">
                        <div className="flex justify-between">
                          <div className="w-1/2 font-semibold overflow-hidden">{cart[item].name} ({cart[item].size}/{cart[item].variant})</div>
                          <div className="bg-slate-100 flex h-8 font-bold text-center px-2"><AiFillMinusCircle onClick={()=>{removeFromCart(item,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> <span className='mx-3'>{cart[item].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(item,cart[item].name,1,cart[item].price,cart[item].size,cart[item].variant)}} className='my-auto cursor-pointer'/> </div>
                        </div>
                        <div>
                        </div>
                        <h1 className='cursor-text text-blue-800 text-xl py-2 rounded'>SubTotal: ${subTotal}</h1>
                      </li>})}
                    </ol>
                ...
                </div> : <h1 className='text-center text-red-700 p-10 font-semibold text-xl'>Please login to see your cart! </h1> }
