              ...
              <div ref={ref} className="pt-3 overflow-scroll cart rounded-lg absolute top-0 right-0 bg-blue-100 px-7 w-2/5 h-full transform transition-transform translate-x-full">
                ...
                <div>
                  <ol className="pt-7 list-decimal font-medium text-red-800 space-y-2">
                    ...
                    {Object.keys(cart).map((k)=>{  return localStorage.getItem('token') ? <h1>Please Login to see your cart! </h1> : <li  key={k} className="text-lg text-blue-800">
                      <div className="flex justify-between">
                        <div className="w-1/2 font-semibold overflow-hidden">{cart[k].name} ({cart[k].size}/{cart[k].variant})</div>
                        <div className="bg-slate-100 flex h-8 font-bold text-center px-2"><AiFillMinusCircle onClick={()=>{removeFromCart(k,cart[k].name,1,cart[k].price,cart[k].size,cart[k].variant)}} className='my-auto cursor-pointer'/> <span className='mx-3'>{cart[k].qty}</span> <AiFillPlusCircle onClick={()=>{addToCart(k,cart[k].name,1,cart[k].price,cart[k].size,cart[k].variant)}} className='my-auto cursor-pointer'/> </div>
                      </div>
                      <div>
                      </div>
                    </li>})}
                      {localStorage.getItem('token') && <h1 className='cursor-text text-blue-800 text-xl py-2 rounded'>SubTotal: ${subTotal}</h1>}
                  </ol>
